...
module Scr = Circuits.MakeSSD1306(I2C)
...
module%comp LeftLed = Circuits.MakeLed(struct let connectedPin = PIN1 end)
module%comp RightLed = Circuits.MakeLed(struct let connectedPin = PIN2 end)
module%comp SPIMaster = MakeSPIMaster(struct let slavePin = PIN0 end)

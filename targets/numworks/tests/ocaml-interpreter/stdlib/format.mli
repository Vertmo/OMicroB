...
(** Pretty-printing.
   ...
   - {!std_formatter} outputs to {{!stdout}stdout}
   - {!err_formatter} outputs to {{!stderr}stderr}
...
(** End of pretty-printing: resets the pretty-printer to initial state.
  ...
  repeated calls to [print_flush] means repeated calls to {!flush}
...
- fields [out_string] and [out_flush] are output device specific;
  (e.g. {!output_string} and {!flush} for a
   ...
   {!ignore} for a [Buffer.t] output device),
...
(** {1:formatter Defining formatters}
  ...
  Similarly, given a {!out_channel} output channel [oc],

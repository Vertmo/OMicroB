...
val fprintf : out_channel -> ('a, out_channel, unit) Format.format -> 'a
...
val printf : ('a, out_channel, unit) Format.format -> 'a
(** Same as {!Printf.fprintf}, but output on [stdout]. *)

val eprintf : ('a, out_channel, unit) Format.format -> 'a
(** Same as {!Printf.fprintf}, but output on [stderr]. *)

val sprintf : ('a, unit, string) Format.format -> 'a
...
val bprintf : Buffer.t -> ('a, Buffer.t, unit) Format.format -> 'a
(** Same as {!Printf.fprintf}, but instead of printing on an output channel,
   append the formatted arguments to the given extensible buffer
   (see module {!Buffer}). *)

val ifprintf : 'b -> ('a, 'b, 'c, unit) Format.format4 -> 'a
...
val kfprintf : (out_channel -> 'd) -> out_channel ->
              ('a, out_channel, unit, 'd) Format.format4 -> 'a
(** Same as [fprintf], but instead of returning immediately,
   passes the out channel to its first argument at the end of printing.
   @since 3.09.0
*)

val ikfprintf : ('b -> 'd) -> 'b -> ('a, 'b, 'c, 'd) Format.format4 -> 'a
(** Same as [kfprintf] above, but does not print anything.
   Useful to ignore some material when conditionally printing.
   @since 4.01.0
*)

val ksprintf : (string -> 'd) -> ('a, unit, string, 'd) Format.format4 -> 'a
...
val kbprintf : (Buffer.t -> 'd) -> Buffer.t ->
              ('a, Buffer.t, unit, 'd) Format.format4 -> 'a
...
val kprintf : (string -> 'b) -> ('a, unit, string, 'b) Format.format4 -> 'a

...
module type SCANNING = sig
  ...
  val from_channel : Numworks.in_channel -> in_channel

  val close_in : in_channel -> unit

  val memo_from_channel : Numworks.in_channel -> in_channel
  (* Obsolete. *)
...
module Scanning : SCANNING = struct
  ...
  type in_channel_name =
    | From_channel of Numworks.in_channel
    | From_file of file_name * Numworks.in_channel
    | From_function
    | From_string
  ...
  *)
  ...
  (* The scanning buffer reading from [stdin].
     ...
     'race condition' when reading from [stdin],
     since the interactive compiler and [Scanf.scanf] will simultaneously
     read the material they need from [stdin]; then, confusion
  ...
  let open_in = open_in_file open_in
  let open_in_bin = open_in_file open_in_bin
  ...
  (*
     ...
     If you need to read from a [Numworks.in_channel] input channel
...
(* Extract an integer literal token.
   Since the functions *int*_of_string do not accept a leading +,

      ...
      (match many (fun xs0 ->
       ...
       | SomeE x0 ->
         ...
         ((fold_left (fun e0 term ->
            let (y, e1) = term in if y then APlus (e0, e1) else AMinus (e0, e1))
            es e), rest')
       | NoneE err -> NoneE err)
    | NoneE err -> NoneE err)
    steps
...
and parseConjunctionExp steps xs =
  (fun zero succ n ->       if n=0 then zero () else succ (n-1))
    ...
    | SomeE x ->
      let (e, rest) = x in
      (match many (firstExpect ('&'::('&'::[])) (parseAtomicExp steps')) steps'
               rest with
       | SomeE x0 ->
         let (es, rest') = x0 in
      ...
      (match firstExpect ('T'::('E'::('S'::('T'::[])))) (parseBExp steps') xs with
       ...
       | NoneE _ ->
         (match firstExpect ('W'::('H'::('I'::('L'::('E'::[])))))
          ...
          | SomeE x ->
            let (e, rest) = x in
            (match firstExpect ('D'::('O'::[])) (parseSequencedCommand steps')
                     rest with
             | SomeE x0 ->
               let (c, rest') = x0 in
               (match expect ('E'::('N'::('D'::[]))) rest' with
